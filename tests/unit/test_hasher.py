...
def test_encode():
    ...
    assert hasher.encode("foo", iterations=99) != hasher.encode(
        "foo", iterations=100
    )
...
def test_str_bytes():
    ...
    assert hasher.verify(
        password_str, hasher.encode(password_str, iterations=iterations)
    )
    assert hasher.verify(
        password_bytes, hasher.encode(password_str, iterations=iterations)
    )
    assert hasher.verify(
        password_str, hasher.encode(password_bytes, iterations=iterations)
    )
    assert hasher.verify(
        password_bytes, hasher.encode(password_bytes, iterations=iterations)
    )
